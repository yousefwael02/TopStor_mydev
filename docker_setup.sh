...
then
	...
	if [ $? -eq 0 ] && [ -f /topstorweb/build_react_new/index.html ];
	then
		rm -rf /topstorweb/build_react
		mv /topstorweb/build_react_new /topstorweb/build_react
	else
		echo "ERROR: React UI build failed, keeping previous /topstorweb/build_react. See /TopStordata/reactbuild.log" >&2
		rm -rf /topstorweb/build_react_new
...
then
 sh /pace/syncrequestlooper.sh $myclusterip $myhost >/TopStordata/syncrequestlooper.log 2>&1 & disown
fi
