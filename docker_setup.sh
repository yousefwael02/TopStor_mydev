...
then
	...
	sed -i "s/MYCLUSTER/$myclusterip/g" $shttpdf
        echo building quickstor-ui:latest from /topstorweb
        docker build -t quickstor-ui:latest /topstorweb > /TopStordata/reactimagebuild.log 2>&1
        imagebuild=$?
	echo building React UI into /topstorweb/build_react
        rm -rf /topstorweb/build_react_new
        mkdir -p /topstorweb/build_react_new
        if [ $imagebuild -eq 0 ];
        then
                docker run --rm -v /topstorweb/build_react_new:/app/build_react quickstor-ui:latest npm run build > /TopStordata/reactbuild.log 2>&1
                reactbuild=$?
        else
                echo "ERROR: quickstor-ui image build failed. See /TopStordata/reactimagebuild.log" >&2
                reactbuild=1
        fi
        if [ $reactbuild -eq 0 ] && [ -f /topstorweb/build_react_new/index.html ];
        then
                rm -rf /topstorweb/build_react
                mv /topstorweb/build_react_new /topstorweb/build_react
        else
                echo "ERROR: React UI update failed, keeping previous /topstorweb/build_react" >&2
                rm -rf /topstorweb/build_react_new
